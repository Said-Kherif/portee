...
3. Read every PNG of the run, both sizes: home, lesson, drill, drill-wrong, drill-grand, exercise, exercise-done, stats, settings. Do not skip the SE folder: overflow, wrapping and clipping show there first.
...
5. Know the expected differences: a fresh headless profile has empty progress, so Home shows every level as new with the first of each group recommended, Stats shows one note after the drill-wrong capture, exercise-done shows a 0 % score because nothing was tapped. These are not findings.
